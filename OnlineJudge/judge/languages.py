...
# - Java: Removed security manager flags (-Djava.security.manager) due to missing /etc/java_policy
# - Golang: Compiler (go 1.19.8) added to JudgeServer/Dockerfile
#
# Language configs are stored in DB (conf_sysoptions.languages) and loaded at runtime.
# To reset DB to these defaults: SysOptions.reset_languages()
...
# Note: Judge-server image currently includes: gcc, g++, python3, java, javac, node
# Go (golang) compiler is NOT included in the current judge-server build
# To enable Go support, add golang package to JudgeServer/Dockerfile
