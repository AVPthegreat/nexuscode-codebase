...
class JudgeDispatcher(DispatcherBase):
    ...
    def judge(self):
        ...
        if resp["err"]:
            ...
            # Clear any stale error info from previous runs on success
            try:
                if "err_info" in self.submission.statistic_info:
                    self.submission.statistic_info["err_info"] = ""
            except Exception:
                # Keep judging result even if statistic_info shape is unexpected
                pass
