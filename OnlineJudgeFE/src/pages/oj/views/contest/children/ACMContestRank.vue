<template>
  <Panel shadow>
    ...
    <Alert v-if="isFrozen && !isContestAdmin" type="warning" show-icon>
      <Icon type="ios-snow" slot="icon"></Icon>
      <span slot="desc">Scoreboard is frozen! Submissions after {{ freezeTimeFormatted }} are hidden until the contest ends.</span>
    </Alert>
...
<script>
  ...
  export default {
    ...
    computed: {
      isFrozen () {
        if (!this.contest.freeze_time) {
          return false
        }
        const now = new Date()
        const freezeTime = new Date(this.contest.freeze_time)
        const endTime = new Date(this.contest.end_time)
      ...
      freezeTimeFormatted () {
        ...
        return moment(this.contest.freeze_time).format('YYYY-MM-DD HH:mm:ss')
      }
    }
  }
</script>
